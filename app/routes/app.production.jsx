...
export async function loader({ request }) {
  ...
  const [productionRuns, products] = await Promise.all([
    ...
    // Only products that have at least one BOM line
    prisma.product.findMany({
      where: { shop, bomLines: { some: {} } },
      orderBy: { name: "asc" },
      include: {
        bomLines: {
          include: {
  ...
  return (
    ...
    >
      <Layout>
        <Layout.Section>
          <InlineStack gap="400" wrap>
            {[
              ...
              { label: "Units Produced", value: productionRuns.reduce((s, r) => s + r.qty, 0) },
              { label: "BOM Products", value: products.length },
            ].map(({ label, value }) => (
              <Card key={label}>
                <BlockStack gap="100">
        ...
        {products.length === 0 && (
          <Layout.Section>
            <Banner tone="warning" title="No products with BOMs yet">
              Go to Bill of Materials, select a product, and add components before logging a production run.
            </Banner>
          </Layout.Section>
        )}
            ...
            {productionRuns.length === 0 ? (
              ...
              <DataTable
                ...
                rows={productionRuns.map(r => [
                  ...
                  <Text fontWeight="semibold">{r.product.name}</Text>,
