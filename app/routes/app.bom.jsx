...
function flattenForDisplay(product) {
  ...
  for (const line of product.bomLines) {
    if (line.material) {
      map[line.materialId] = map[line.materialId] || { id: line.materialId, name: line.material.name, unit: line.material.unit, cost: line.material.cost, qty: 0, via: null };
      map[line.materialId].qty += line.qty;
    } else if (line.subAssembly) {
      for (const comp of line.subAssembly.components) {
        map[comp.materialId] = map[comp.materialId] || { id: comp.materialId, name: comp.material.name, unit: comp.material.unit, cost: comp.material.cost, qty: 0, via: line.subAssembly.name };
...
function QtyEditor({ line, onSave }) {
  ...
  if (!editing) {
    return (
      <InlineStack gap="100" blockAlign="center">
        <Text>{line.qty}</Text>
        <Button size="slim" variant="plain" onClick={() => setEditing(true)}>Edit</Button>
      </InlineStack>
    );
  }
  return (
    <InlineStack gap="100" blockAlign="center">
      <div style={{ width: "70px" }}>
        <TextField value={val} onChange={setVal} type="number" label="" labelHidden autoComplete="off" />
      </div>
      <Button size="slim" variant="primary" onClick={() => { onSave(val); setEditing(false); }}>Save</Button>
      <Button size="slim" variant="plain" onClick={() => { setVal(String(line.qty)); setEditing(false); }}>Cancel</Button>
    </InlineStack>
  );
}
        ...
        <Layout.Section variant="oneThird">
          <Card>
            <BlockStack gap="300">
              ...
              <Divider />
              {products.length === 0 ? (
                <Text tone="subdued" variant="bodySm">Sync your Shopify products first using the button above.</Text>
              ) : filteredProducts.length === 0 ? (
                <Text tone="subdued" variant="bodySm">No products match your search.</Text>
              ) : (
                <BlockStack gap="200">
                  ...
                  {selectedProductData.bomLines.length === 0 ? (
                    ...
                    <DataTable
                      ...
                      rows={selectedProductData.bomLines.map(line => {
                        ...
                        const lineCost = line.material
                          ? line.material.cost * line.qty
                          : getSubAsmCost(line.subAssembly) * line.qty;
                        return [
                          <Text fontWeight="medium">{name}</Text>,
                          <Badge tone={isSubAsm ? "attention" : "info"}>{isSubAsm ? "Sub-Assembly" : "Raw Material"}</Badge>,
                          <QtyEditor
                            line={line}
                            onSave={(newQty) => {
                              const fd = new FormData();
                              fd.append("intent", "updateBomLineQty");
                ...
                <BlockStack gap="300">
                  ...
                  <InlineStack gap="300" blockAlign="end">
                    ...
                    <Box paddingBlockStart="500">
                      <Button
                        disabled={!bomMatForm.materialId}
                        onClick={() => {
                          const fd = new FormData();
                  ...
                  {subAssemblies.length > 0 && (
                    <InlineStack gap="300" blockAlign="end">
                      ...
                      <Box paddingBlockStart="500">
                        <Button
                          disabled={!bomSubForm.subAssemblyId}
                          onClick={() => {
                            const fd = new FormData();
